...
| **14** `audit` | `vendor/themekit/cmd/mimber/audit.go` + `scripts/audit.mjs` — 13 rules (`jquery-ajax`/`handlebars`/`locale-fetch`/`liquid-img-url-legacy`/`liquid-legacy-img-src`/…), score 0–100 | `npm run audit` / `audit:json` + `go run ./cmd/mimber audit --json` → `audit.json` (excludes frozen `assets/timber.js.liquid`, `asset_url` logos) |
| **15** `Liquid images` | `templates/product.liquid` (hero `eager` + thumbs `lazy`) + `snippets/product-grid-item.liquid` + `templates/{collection,blog,cart,search,index}` + `snippets/social-meta-tags.liquid` — `img_url:'large'` → `image_url: width: 800/400` + `image_tag` | `loading:'lazy'`/`eager fetchpriority:'high'` + `widths` srcset, `image_tag` auto `width`/`height`/`srcset` per https://shopify.dev/docs/api/liquid/filters/image_tag |
| **16** `LiquidJS sanity` | `scripts/liquid-check.mjs` + `tests/unit/liquid.test.js` — [liquidjs](https://liquidjs.com) https://github.com/harttle/liquidjs, mocks `image_url`/`image_tag`/`paginate`/`form`/`render` + dummy `product`/`cart`/`settings` context | `npm run liquid:check` / `liquid:check:json` + `go run ./cmd/mimber liquid --json` → `liquid-audit.json` (42/42 ok); `vitest` 3 cases |
...
| `templates/product.liquid` + `snippets/product-grid-item.liquid` + `templates/{collection.list,cart,search,blog,index}` | Modern `image_url`/`image_tag` + native lazy (slice 15) | `templates/product.liquid` | `image_url: width: 800` + `image_tag loading:'eager'/'lazy'`, `widths` srcset; replace `img_url: 'large'` everywhere |
| `layout/theme.liquid` | Now `mepto.js` + `base.css` + `css-variables` (was `jquery/1.12.4` + `handlebars` + `fastclick/modernizr` + `timber.scss.css`) | `layout/theme.liquid` | Already `mepto.js`+`base.css`; remove `timber.scss.css`/`jquery` tags, add `css-variables`; also `Shopify.routes.root` for locale (slice 11) |
