...
export const loginForms = (timber) => {
  ...
  const hideRecover = () => {
    scheduler.mutate(() => {
      const af =
        timber.cache.$recoverPasswordForm &&
        (timber.cache.$recoverPasswordForm[0] || timber.cache.$recoverPasswordForm)
      const bf =
        timber.cache.$customerLoginForm &&
        (timber.cache.$customerLoginForm[0] || timber.cache.$customerLoginForm)
